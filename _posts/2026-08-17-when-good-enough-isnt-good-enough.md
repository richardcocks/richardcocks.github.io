...
When working with claude, this is made so much more difficult. You can get to the end of the work, consider it finished, then ask claude for one final look over, and back it comes with a dozen minor code issues, some of which you'd never have even considered, such as the possibility that if someone calls this function in a particular state, then quits at just the right time, they could end up in a loop. Well that sounds bad, we can't ship with that!

While doing a final human developer testing pass, you realise it'd be handy to add a particular hotkey. Well surely that can't take long, we better quickly add it. Surely it'll only take one prompt and a few minutes.
